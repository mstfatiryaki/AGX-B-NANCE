...
"""
SentientTrader.AI - Long Strategy V2
------------------------------------
This module implements a long trading strategy that enters positions
on oversold conditions with increasing volume and positive sentiment.
"""
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("long_strategy.log")]
)
logger = logging.getLogger("LongStrategy")
...
CURRENT_TIME = "2025-04-21 21:05:03"  # UTC
...
TRADING_PARAMS = {
    ...
    "leverage": 1,  # No leverage for long strategy
    ...
    "min_sentiment_score": 0.0,  # Minimum sentiment score to consider
    "confidence_threshold": 0.7,  # Minimum strategy confidence to execute
}

class LongStrategy:
    """Implements a long trading strategy for cryptocurrency markets"""
    
    def __init__(self, real_mode: bool = REAL_MODE, simulation_mode: bool = SIMULATION_MODE):
        """
        Initialize the long strategy
        ...
        logger.info(f"Long Strategy initialized (REAL_MODE: {self.real_mode}, SIMULATION_MODE: {self.simulation_mode})")
    ...
    def load_strategy_decision(self) -> bool:
        ...
        try:
            ...
            if strategy_name != "long_strategy.py" and not self.simulation_mode:
                logger.info(f"Current strategy is {strategy_name}, not long_strategy.py. Exiting.")
                return False
        ...
        try:
            for coin, data in self.market_data.items():
                ...
                if high == low:
                    ...
                    # Invert since RSI is lower when price is at the low end of the range
                    rsi = 100 - (price_position * 100)
                
                # Volume increase indicator (mock - in a real system, this would use historical data)
                volume_increase = random.uniform(0.9, 1.5)  # Mock value for simulation
    ...
    def check_entry_conditions(self, coin: str) -> Tuple[bool, float, Dict[str, Any]]:
        """
        Check if entry conditions are met for a coin
        ...
        try:
            ...
            # 1. Check RSI (oversold condition)
            rsi = indicators.get("rsi", 50)
            rsi_condition = rsi <= TRADING_PARAMS["min_rsi"]
            conditions_met["rsi_oversold"] = rsi_condition
            
            # 2. Check volume increase
            volume_increase = indicators.get("volume_increase", 1.0)
            volume_condition = volume_increase >= TRADING_PARAMS["min_volume_increase"]
            conditions_met["volume_increasing"] = volume_condition
            
            # 3. Check sentiment (if available)
            sentiment_condition = sentiment_score >= TRADING_PARAMS["min_sentiment_score"]
            conditions_met["positive_sentiment"] = sentiment_condition
            
            # 4. Price is in uptrend or turning point (simplified check)
            price_change = market_data.get("price_change_24h", 0)
            price_to_ma = indicators.get("price_to_ma_ratio", 1.0)
            uptrend_condition = price_change > -5.0 and price_to_ma >= 0.95
            conditions_met["potential_uptrend"] = uptrend_condition
            
            # Calculate overall confidence score (weighted average of conditions)
            weights = {
                "rsi_oversold": 0.35,
                "volume_increasing": 0.25,
                "positive_sentiment": 0.2,
                "potential_uptrend": 0.2
            }
    ...
    def calculate_position_size(self, coin: str, available_balance: float) -> float:
        ...
        if coin in self.technical_indicators and coin in self.market_data:
            ...
            # Adjust for sentiment (increase size for positive sentiment)
            if coin in self.sentiment_data:
                sentiment_score = self.sentiment_data[coin].get("sentiment_score", 0)
                if sentiment_score > 0.5:
                    max_position *= (1.0 + (sentiment_score - 0.5))
    ...
    def calculate_entry_exit_levels(self, coin: str) -> Dict[str, float]:
        ...
        # Calculate stop loss level (default 5% below entry)
        stop_loss = entry_price * (1 - TRADING_PARAMS["stop_loss_percentage"])
        
        # Calculate take profit level (default 15% above entry)
        ...
        if coin in self.technical_indicators:
            ...
            rsi = indicators.get("rsi", 50)
            if rsi < 20:
                # Wider stop-loss for extremely oversold conditions
                stop_loss = entry_price * (1 - (TRADING_PARAMS["stop_loss_percentage"] * 1.2))
            
            # Adjust take profit based on bollinger bands
            bollinger_pos = indicators.get("bollinger_band_position", 0)
            if bollinger_pos < -0.8:
                # Higher take-profit for more oversold coins
    ...
    def execute_trade(self, coin: str, levels: Dict[str, float], 
        ...
        """
        # Generate trade ID
        trade_id = f"LONG_{int(time.time())}_{coin}_{uuid.uuid4().hex[:8]}"
            ...
            try:
                ...
                if not allocation_result.get("success", False):
                    ...
                    if ALERT_SYSTEM_AVAILABLE:
                        alert_system.error(
                            f"Fund allocation failed for {coin} long trade",
                            {
                                "coin": coin,
                ...
                logger.info(f"Allocated ${allocation:.2f} for {coin} long trade")
                ...
                if ALERT_SYSTEM_AVAILABLE:
                    alert_system.error(
                        f"Fund allocation error for {coin} long trade",
                        {
                            "coin": coin,
                            "allocation": allocation,
                            "error": str(e)
        ...
        # In simulation mode or if capital manager not available, mock allocation
            ...
            logger.info(f"[SIMULATION] Allocated ${allocation:.2f} for {coin} long trade")
        ...
        if MEMORY_CORE_AVAILABLE:
            memory_core.add_memory_record(
                source="long_strategy",
                category="trade_execution",
                data=trade,
                tags=["trade", "long", coin, "entry"]
            )
    ...
    def close_trade(self, trade: Dict[str, Any], exit_price: float, exit_reason: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.simulation_mode and CAPITAL_MANAGER_AVAILABLE:
                try:
                    ...
                    if not release_result.get("success", False):
                        ...
                        if ALERT_SYSTEM_AVAILABLE:
                            alert_system.error(
                                f"Fund release failed for {coin} long trade",
                                {
                                    "coin": coin,
                    ...
                    if ALERT_SYSTEM_AVAILABLE:
                        alert_system.error(
                            f"Fund release error for {coin} long trade",
                            {
                                "coin": coin,
                                "investment": investment,
                                "pnl": pnl,
            ...
            logger.info(f"Closed LONG trade for {coin} at ${exit_price:.6f}")
            ...
            if ALERT_SYSTEM_AVAILABLE:
                ...
                alert_func(
                    f"LONG position closed for {coin} with {pnl:.2f} USD {pnl >= 0 and 'profit' or 'loss'}",
                    {
                        "trade_id": trade_id,
                        "coin": coin,
            ...
            if MEMORY_CORE_AVAILABLE:
                memory_core.add_memory_record(
                    ...
                    category="trade_execution",
                    data=trade,
                    tags=["trade", "long", coin, "exit", exit_reason]
                )
    ...
    def execute_strategy(self) -> bool:
        """
        Execute the strategy for selected coins
        ...
        try:
            ...
            logger.info(f"Executing long strategy for {len(target_coins)} coins")
            ...
            # Log summary
            if executed_trades:
                logger.info(f"Executed {len(executed_trades)} trades, allocated ${total_allocated:.2f}")
                
                # Terminal summary
                print(f"\n{COLORS['bright']}{COLORS['green']}LONG Strategy Execution Summary{COLORS['reset']}")
                ...
                for trade in executed_trades:
                    ...
                    print(f"\n{COLORS['bright']}{coin} LONG @ ${entry:.6f}{COLORS['reset']}")
                ...
                print(f"\n{COLORS['yellow']}No long trades executed. Entry conditions not met for any target coins.{COLORS['reset']}")
        ...
        try:
            ...
            if not self.load_strategy_decision():
                if not self.simulation_mode:
                    logger.info("Strategy decision not applicable to long strategy, exiting")
                    return False
                else:
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="SentientTrader.AI Long Strategy")
...
def main() -> int:
    ...
    try:
        # Initialize strategy
        strategy = LongStrategy(real_mode=real_mode, simulation_mode=simulation_mode)
        ...
        if args.coin:
            ...
            print(f"{COLORS['cyan']}Running long strategy for {coin} with ${allocation:.2f} allocation{COLORS['reset']}")
            ...
            if entry_decision:
                ...
                if trade["status"] == "OPEN":
                    print(f"{COLORS['green']}Successfully opened LONG position for {coin}{COLORS['reset']}")
                else:
        ...
        # Otherwise run full strategy process
            ...
            if simulation_mode:
                print(f"{COLORS['cyan']}Running long strategy simulation{COLORS['reset']}")
                sample_coins = ["BTC", "ETH", "SOL", "LINK", "ADA"]
                strategy.simulate_trades(sample_coins[:3])  # Use first 3 coins
                
            # Otherwise run normal process
            else:
                print(f"{COLORS['cyan']}Running long strategy{COLORS['reset']}")
                strategy.run()
